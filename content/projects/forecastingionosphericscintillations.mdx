...
This project addresses the challenge of predicting signal disturbances affecting satellite communications over the Indian subcontinent. I researched and implemented LSTM, X-LSTM in Pytorch+Pytorch Lightning and Graphcast in Jax+Flax on a very limited dataset.
...
The architecture emphasizes modularity, with separate components for data loading, visualization, and model training that can be extended or modified independently. Despite dataset limitations, the models demonstrate promising capabilities for short-term scintillation forecasting.

The major bottleneck in model performance was that I only had data from one station, therefore making a horrible grid of points.
